...
- One new endpoint, not several granular ones, because it matches the one view the UI actually needs:
...
- **It needs a constraint the generic table can't express**, like "at most one `assigned_rep` per subscription", which needs a real unique index scoped to the key, which the current table has no clean way to do.
...
- Stripe "not found" (customer/subscription doesn't exist) is distinct from "not found" on the D1 side, and is surfaced as a clear, specific message, not a generic error.

...
`src/flag-rules.ts` exports `FLAG_RULES`, an array of
...
`node:test` run through `tsx`, `globalThis.fetch` stubbed, and a fake D1
binding (the repo has no D1 test double yet; this adds a small one). Cover:
...
The design above is what was agreed before code. Four things moved, three of
them because review found a bug the design did not anticipate. They are
recorded here so this document describes what shipped:

- **`processed_messages` carries the watermark, not just idempotency.** It
  gained a `received_at` column and the next poll reads
  `MAX(received_at)` from it. There is no separate cursor row that can fall
  out of step with what was actually processed — which also means the table
  must never be pruned. See the comment on it in `db/schema.ts`.
- **The poll reads the Inbox, not the whole mailbox.** `/users/{id}/messages`
  spans Sent Items, so every reply the team sent ("Re: refund request") was a
  message whose sender was the support mailbox — flagging the mailbox as an
  account and pinning it to the top of its own queue. Scoped to
  `/mailFolders/inbox/messages`, plus an independent sender check in
  `src/flagging.ts` because the folder scoping is unverified against a live
  tenant. `listCorrespondence` still reads the whole mailbox on purpose.
- **The poll asks for immutable message IDs.** Graph IDs are not stable:
  filing a message into a folder reassigns the ID, and idempotency is keyed on
  it. So handle ticket → clear flag → file the mail → flag returns on the next
  poll. `Prefer: IdType="ImmutableId"` closes that, and is itself listed as an
  unverified item in `docs/office365-mail-setup.md`.
- **`FlagRule` is a type, so the body refusal is compiler-enforced.** A rule
  receives a `ShapedMessage`, which declares no body field: a rule that reaches
  for one does not compile, rather than reading `undefined` at runtime.

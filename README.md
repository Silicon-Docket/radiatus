...
| `src/types.ts` | The `Env` interface and the `isGraphConfigured` / `isStripeConfigured` predicates that make an unconfigured feature uncallable |
| `src/flag-rules.ts` | The rules that decide which support emails flag an account — **edit this**; it ships with one example |
| `src/flagging.ts` | The scheduled poll: fetch new mail, evaluate rules, flag accounts, remember what was seen |
| `db/schema.ts` | Drizzle schema — the source of truth for the D1 tables |
| `migrations/` | Generated by `npm run db:generate`; don't hand-edit |
| `test/worker.test.ts` | Node test-runner tests for the Worker's request handlers |
| `test/stripe.test.ts` | Tests for the Stripe client, with mocked HTTP responses |
| `test/graph.test.ts` | Tests for the Graph client — token caching, the metadata whitelist, the sender-only fallback |
| `test/flagging.test.ts` | Tests for the flag rules and the polling logic |
| `test/fake-d1.ts` | A strict D1 test double — rejects SQL it doesn't recognise instead of faking success |
...
| `tsconfig.json` | TypeScript config — typecheck only (`noEmit`); Wrangler compiles the Worker |
...
`db/schema.ts` (Drizzle ORM, `drizzle-orm/sqlite-core`) is the source of truth for the D1 table shapes. It's used only to generate migration SQL, not as a runtime query layer — `src/worker.ts` still talks to D1 directly. To change the schema: edit `db/schema.ts`, run `npm run db:generate` to write a new file under `migrations/`, then `npm run db:migrate:local` (or `db:migrate:remote`) to apply it.
...
What flags an account is yours to decide, in [`src/flag-rules.ts`](./src/flag-rules.ts). It ships with exactly one rule, marked as an example to replace: a subject containing the word "refund". Export an empty array and flagging is off while everything else keeps working.

**Rules match on the message subject and sender address only. Never the body.** That is not a limitation waiting to be lifted: the Exchange grant this integration asks for (`Application Mail.ReadBasic`) excludes message content at the grant, so bodies are never fetched in the first place. Matching on body text would mean upgrading to `Mail.Read`, at which point a leaked `ADMIN_API_TOKEN` could read every message in the mailbox. A rule that needs body text cannot be written here, on purpose &mdash; the `ShapedMessage` type a rule receives has no field for one, so such a rule does not compile.
...
npm test            # node:test, run through tsx
```

There is no build step. `tsc` only typechecks (`noEmit`) — Wrangler compiles `src/worker.ts` on `dev` and `deploy`, and `tsx` compiles the tests on the fly. All three commands run in CI.

...
import { pollAndFlag } from './flagging.js';
...
<html lang="en">
  ...
  <body>
    ...
    <p class="muted">
      ...
      fetched, by design. Edit <code>src/flag-rules.js</code> to change what raises a flag.
 ...
 * widening the view.
 */
export function parseFlaggedOnly(rawValue) {
 ...
 */
function listAccounts(db, { q, flaggedOnly }) {
  return db.prepare(
    `SELECT ${ACCOUNT_COLUMNS}
     ...
     LIMIT 200`
  ).bind(flaggedOnly ? 1 : 0, q ? likePattern(q) : '%').all();
}
 ...
 */
function clearAccountFlag(db, email) {
  return db.prepare(
    `UPDATE accounts
     SET flagged = 0
     WHERE email = ?1

import type { ShapedMessage } from './graph';

/**
 * Rules that decide which support-mailbox messages flag an account.
 *
 * WHAT A RULE CAN SEE: metadata only — the shaped message from src/graph.ts,
 * whose type is `ShapedMessage`: `{ id, subject, from, toRecipients,
 * receivedDateTime, webLink, conversationId, hasAttachments }`. In practice
 * that means the SUBJECT and the SENDER ADDRESS.
 *
 ...
 * cannot be expressed here, deliberately: `ShapedMessage` has no field for it,
 * so such a rule does not compile.
 *
 ...
 *
 * A rule that throws is caught and skipped by the caller, so a typo in one
 * regex cannot stop the whole run — but it also means a broken rule fails
 * silently, so test yours.
 */
export interface FlagRule {
  /** Stored as the account's `flag_reason`, so an operator sees which rule fired. */
  id: string;
  /** For whoever reads this file next. Required so a rule cannot arrive unexplained. */
  description: string;
  /** Return true to flag the sender's account. */
  matches(message: ShapedMessage): boolean;
}
 ...
 */
const REFUND_EXAMPLE_RULE: FlagRule = {
  id: 'refund-mention',
...
export const FLAG_RULES: FlagRule[] = [REFUND_EXAMPLE_RULE];
